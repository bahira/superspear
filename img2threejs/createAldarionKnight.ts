...
// Generated from ObjectSculptSpec target: Aldarion Knight
// Sculpt build pass: blockout
// This factory is intentionally pass-gated. Finish browser screenshot review before unlocking deeper passes.
export function createAldarionKnightModel(options: ProceduralModelOptions = {}): THREE.Group {
  ...
  const mesh_armR_3Geometry = endpoint_armR_3
    ? new THREE.CylinderGeometry(endpoint_armR_3.endRadius, endpoint_armR_3.baseRadius, endpoint_armR_3.length, 32, 12)
  ...
  const mesh_armL_4Geometry = endpoint_armL_4
    ? new THREE.CylinderGeometry(endpoint_armL_4.endRadius, endpoint_armL_4.baseRadius, endpoint_armL_4.length, 32, 12)
    : buildWatertightCapsule(0.35, 0.7, 16, 32, 1);
  if (!endpoint_armL_4) {
  ...
  const mesh_legs_5Geometry = endpoint_legs_5
    ? new THREE.CylinderGeometry(endpoint_legs_5.endRadius, endpoint_legs_5.baseRadius, endpoint_legs_5.length, 32, 12)
    : buildWatertightCapsule(0.35, 0.7, 16, 32, 1);
  if (!endpoint_legs_5) {
  ...
  const mesh_gear_6 = new THREE.Mesh(
    mesh_gear_6Geometry,
    materialMap["clothNavy"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
...
export function createAldarionKnightLookDevLights(
...
export function createAldarionKnightEnvironment(renderer: THREE.WebGLRenderer): THREE.Texture {
...
export function frameAldarionKnightCamera(
...
export function createAldarionKnightPresentationComposer(
...
export function configureAldarionKnightRenderer(renderer: THREE.WebGLRenderer): void {
...
export function createAldarionKnightInspectControls(
  camera: THREE.Camera,
